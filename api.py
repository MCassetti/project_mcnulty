...
def make_prediction(features):
    ...
    prob_metallica = pipeline.predict_proba(X)[0, 1]
...
def make_and_extract():
    ...
    try:
        ...
        if num_slices >= 2:
            mid_point = math.floor(num_slices/2)
        else:
            mid_point = 1
        print("mid_point:",mid_point)
        y = y_total[frames[int(mid_point)] + rand_offset:frames[int(mid_point) + 1] + rand_offset ] #offset in case something in training set
    except:
